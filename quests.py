        ...
        if test_soup.find("h1", {"class": "heading-size-1"}).text not in ["Quests", "<UNUSED>"]:
            ...
            for li, l in enumerate(language):
                ...
                with open(filename, mode='a', encoding='utf-8-sig') as csv_output:
                            ...
                            while True:
                                ...
                                if isinstance(nextNode, Tag):
                                    if nextNode.name == "h2" or nextNode.name == "table":
                                        break
                                    details += nextNode.get_text(
                                        strip=True).strip()
                    except:
                        details = ""
                    details = re.sub(r'([.?!])([a-zA-Z])',
                                     r'\1$B$B\2', details)

                    # get id objectives
                    try:
                        objectives = soup.find(
                            "div", {"class": "block-block-bg is-btf"}).next_sibling.strip()
                    except:
                        objectives = ""
                    objectives = re.sub(
                        r'([.?!])([a-zA-Z])', r'\1$B$B\2', objectives)

                    # get id offer reward text
                    try:
                        offerRewardText = soup.find(
                            "div", {"id": "lknlksndgg-completion"}).text.strip()
                    except:
                        offerRewardText = ""
                    offerRewardText = re.sub(
                        r'([.?!])([a-zA-Z])', r'\1$B$B\2', offerRewardText)

                    # get id requestItemsText
                    try:
                        requestItemsText = soup.find(
                            "div", {"id": "lknlksndgg-progress"}).text.strip()
                    except:
                        requestItemsText = ""
                    requestItemsText = re.sub(
                        r'([.?!])([a-zA-Z])', r'\1$B$B\2', requestItemsText)
                    ...
                    try:
                        objectiveText1 = soup.find("table", {"class": "iconlist"}).select("tr")[
                            0].text.strip()
                    except:
                        objectiveText1 = ""
                    ...
                    try:
                        objectiveText2 = soup.find("table", {"class": "iconlist"}).select("tr")[
                            1].text.strip()
                    except:
                        objectiveText2 = ""
                    ...
                    try:
                        objectiveText3 = soup.find("table", {"class": "iconlist"}).select("tr")[
                            2].text.strip()
                    except:
                        objectiveText3 = ""
                    ...
                    try:
                        objectiveText4 = soup.find("table", {"class": "iconlist"}).select("tr")[
                            3].text.strip()
                    except:
                        objectiveText4 = ""
                    ...
                    if print_debug:
                        ...
                        print("[" + l + "] requestItemsText: " +
                              requestItemsText)
                        print("[" + l + "] endText: " + endText)
                        print("[" + l + "] objectiveText1: " +
                              str(objectiveText1))
                        print("[" + l + "] objectiveText2: " +
                              str(objectiveText2))
                        print("[" + l + "] objectiveText3: " +
                              str(objectiveText3))
                        print("[" + l + "] objectiveText4: " +
                              str(objectiveText4))
                    ...
                    for index, item in enumerate(wowhead_format):
                        ...
                        details = details.replace(
                            item[li], mangos_format[index])
                        objectives = objectives.replace(
                            item[li], mangos_format[index])
                        offerRewardText = offerRewardText.replace(
                            item[li], mangos_format[index])
                        requestItemsText = requestItemsText.replace(
                            item[li], mangos_format[index])

                    # write to csv
                    csv_writer.writerow([i, title, details, objectives, offerRewardText, requestItemsText,
                                        endText, objectiveText1, objectiveText2, objectiveText3, objectiveText4])
                    csv_output.close()
            print("=========")
print("DONE")
